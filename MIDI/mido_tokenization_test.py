...
def get_token_from_midi(mid, bpm = BPM, ticks_per_beat = TICKS_PER_BEAT, beat_per_bar = BEAT_PER_BAR):
    ...
    tick_duration = tick2second(1, ticks_per_beat, tempo)

    print(beat_duration, ticks_per_bar, tempo, tick_duration)
    ...
    for msg in mid.play():        
        if msg.type == 'note_on':
            ...
            logging.info(msg)
    if len(tokens) < ticks_per_bar:
        for i in range(ticks_per_bar - len(tokens)):
            tokens.append(SILENCE_TOKEN)
    bars.append(tokens)
...
if __name__ == "__main__":
    ...
    mid = mido.MidiFile(os.path.join(MIDI_FOLDER_PATH, 'examples/samples.MID'), type=1)
    ...
    save_path = os.path.join(MIDI_FOLDER_PATH, 'output/test.mid')
